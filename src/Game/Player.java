...
public class Player {
	...
	int currentForm = 0;
	int walkAnimFrames=17;
	int walkingSpeed = 512/walkAnimFrames;//512
	...
	public void enterBattle(int monsterID, int color, boolean wild){
		int max = GamePanel.rooms.get(GamePanel.currentRoom).maxLevel;
		int min = GamePanel.rooms.get(GamePanel.currentRoom).minLevel;
		currentTarget = new Creature(GamePanel.randomNumber(min,max),monsterID,color,wild);
		battling = true;
	...
	public void Draw(Graphics g){
		if(!GamePanel.editMode){
			...
			if(walkAnim!=null){
				...
				if(walkAnim!=null){
					if(direction==1){
						xChange = walkAnim.framesDrawn*2;
					}
					else if(direction==2){
						xChange = -walkAnim.framesDrawn*2;
					}
					...
					if(direction==0){
						yChange = walkAnim.framesDrawn*2;
					}
					else if(direction==3){
						yChange = -walkAnim.framesDrawn*2;
					}
				}
				int x2 = 944+(xpos*32)-(xChange+xpos*32);
				int y2 = 514+(ypos*32)-(yChange+ypos*32);
				...
				if(walkAnim.framesDrawn>=walkAnim.getFrameCount()){

					if(direction==2){
						if(AppletUI.GAMEMODE==AppletUI.MULTIPLAYER){
							//AppletUI.client.moveLeft();
						}
						else{
							xpos-=1;
						}
				...
				if(name==GamePanel.name||true){
					...
					int y2 = 514+(ypos*32)-(yChange+ypos*32);
					if(GamePanel.spiritWorldOn==false){
						g.drawImage(artwork[direction][0],x2,y2,32,32,null);
					}
					else{
						g.drawImage(GamePanel.playerSpritSprites[direction][0],x2,y2,32,32,null);
					}
				}
